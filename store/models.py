...
class Category(models.Model):
    ...
    image = models.FileField(upload_to=user_directory_path, default="category.jpg", null=True, blank=True)
...
class Product(models.Model):
    ...
    def OutdoorImage(self):
        return OutdoorImage.objects.filter(product=self)
    
    def OutdoorColor(self):
        return OutdoorColor.objects.filter(product=self)
...
# Adding OutdoorImage and OutdoorColor models as inline models for Product
class OutdoorImage(models.Model):
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    image = models.ImageField(upload_to='outdoor_images/')
class OutdoorColor(models.Model):
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    color_name = models.CharField(max_length=100)
    color_code = models.CharField(max_length=7)

class MobileImage(models.Model):
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='mobile_images')
